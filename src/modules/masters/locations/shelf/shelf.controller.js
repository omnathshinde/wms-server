...
export const bulkCreate = async (req, res) => {
	...
	// ---------- LOOKUPS ----------
	const siteNames = [
		...new Set(shelves.map((s) => s.siteName?.trim()).filter(Boolean)),
	];
	const zoneNames = [
		...new Set(shelves.map((s) => s.zoneName?.trim()).filter(Boolean)),
	];
	const rackNames = [
		...new Set(shelves.map((s) => s.rackName?.trim()).filter(Boolean)),
	];
