...
export const update = async (req, res) => {
	...
	if (picklist.isIssued && isIssued) {
		return res.sendError(
			400,
			"Cannot modify a picklist that has already been issued.",
		);
	}
	...
	if (picklistStatus === "Completed") {
		...
		if (pickedItems === 0) {
			return res.sendError(
				400,
				"Pick at least one item before marking as completed",
			);
		}
	...
	if (
		...
		if (inwardRows === 0) {
			return res.sendError(
				404,
				"No inward barcodes were updated for this picklist",
			);
		}
